            ...
            try {
                ...
                if ($jsonObject.ValidJson) {
                    ...
                    foreach ($project in $Projects) {
                        ...
                        [string]$apiTaskgroupPostUrl = Get-TaskgroupPostUrl -AdoServerUrl $ServerUrl -Project $project
                        [string]$apiTaskgroupGetUrl = Get-TaskgroupGetUrl -AdoServerUrl $ServerUrl -Project $project
                        ...
                        if ($taskGroupList.count -eq 0 -or -not ($uniqueListOfTaskgroups | Where-Object -FilterScript {$_.name -eq $jsonObjectName})) {
                            ...
                            try {
                                ...
                                if($true -eq $DenyContributorEditPermission)
                                {
                                    ...
                                    [PSCustomObject]$responseObject = Set-TaskGroupPermissions -IsAdoCloud $IsAdoCloud -AdoServerUrl $ServerUrl -Project $project -TaskgroupName $jsonObjectName -RequestSleepTime $RequestSleepTime
                            ...
                            if ($jsonObjectName -eq $remoteTaskgroup.name) {                                 
                                try {
                                        ...
                                        if($jsonTaskgroupObject.PSObject.Properties['isMinorChange'] -and $jsonTaskgroupObject.isMinorChange -eq $false) {
                                            ...
                                            [string]$apiTaskgroupPutUrl = Get-TaskgroupPutUrl -AdoServerUrl $ServerUrl -Project $project -ParentTaskgroupId $remoteTaskgroup.id
                                            ...
                                            [string]$apiTaskgroupPatchUrl = Get-TaskgroupGetUrl -AdoServerUrl $ServerUrl -Project $project -TaskgroupId $remoteTaskgroup.id -DisablePriorVersions 'false'
                                            $changeSet = @{
                                            ...
                                            [psobject]$updatedJsonContent = $jsonTaskgroupObject | ConvertTo-Json -Depth 10 -Compress
                                            [string]$apiTaskgroupPutUrl = Get-TaskgroupPutUrl -AdoServerUrl $ServerUrl -Project $project -TaskgroupId $remoteTaskgroup.id
